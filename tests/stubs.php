<?php
/**
 * The core's surface, as far as the add-ons touch it.
 *
 * One file for all ten. It used to be ten identical copies of these three
 * hundred and sixty-four lines, one per repository — the same arrangement that
 * put ten copies of the add-on boilerplate in ten places and made one defect
 * into ten. What is *not* stubbed here is WP2Static\Addon\: those classes are
 * the core's real ones, pulled in as a dev dependency, so a change to the base
 * that breaks an add-on fails this suite instead of a user's site.
 *
 * In production the core is a runtime dependency and not a Composer one: it is
 * installed alongside, not required, and an add-on ships with no `vendor/` at
 * all. For tests it is a dev dependency, which is how WP2Static\Addon\ is real
 * here. These stubs cover the rest of the core's surface — the parts an add-on
 * calls but should not have to run a WordPress to exercise.
 *
